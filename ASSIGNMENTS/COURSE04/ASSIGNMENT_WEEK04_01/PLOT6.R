...
###############################################################################################
#PLOTTING
ggplot(cities_comparison,aes(factor(year),Emissions,fill=city)) +
    geom_bar(stat="identity") +
    guides(fill=FALSE)+
    facet_wrap(~ city)+
    geom_smooth(aes(group=1), method=lm, col="black", linewidth=1.5,alpha=0.3) +
    labs(x="year", y=expression("Total Emission, in T")) + 
    labs(title=expression("PM"[2.5]*" Vehicle Emissions in Baltimore and LA, 1999-2008, compared"))
dev.off()